...
namespace FireflyIIINet.Api
{
    public interface ITransactionsApi
    {
        ...
        Task<AttachmentArray> ListAttachmentByTransaction(string id, [Header("X-Trace-Id")] Guid? xTraceId = null, int? page = null);

        [Get("/v1/transactions/{id}/piggy-bank-events")]
        Task<PiggyBankEventArray> ListEventByTransaction(string id, [Header("X-Trace-Id")] Guid? xTraceId = null, int? page = null);

        [Get("/v1/transaction-journals/{id}/links")]
        Task<TransactionLinkArray> ListLinksByJournal(string id, [Header("X-Trace-Id")] Guid? xTraceId = null, int? page = null);

        [Get("/v1/transactions")]
        Task<TransactionArray> ListTransaction([Header("X-Trace-Id")] Guid? xTraceId = null, int? page = null, [Query(Format = "yyyy-MM-dd")] DateTime? start = null, [Query(Format = "yyyy-MM-dd")] DateTime? end = null, TransactionTypeFilter? type = null);
